...
export const PASSAGES: Passage[] = [
  ...
  { difficulty: 'hard', text: 'Quantum entanglement — described by Einstein as "spooky action at a distance" — enables correlations between particles that persist regardless of separation. This phenomenon underpins quantum cryptography & teleportation protocols.' },
  { difficulty: 'hard', text: 'The Curry-Howard correspondence establishes a deep isomorphism between computer programs and mathematical proofs: types correspond to propositions, and programs correspond to proofs. This duality bridges logic & computation.' },
  ...
  { difficulty: 'hard', text: 'CRISPR-Cas9 gene editing leverages bacterial immune mechanisms to make precise modifications to DNA sequences. Off-target effects remain a significant concern; specificity depends on guide RNA complementarity & PAM recognition.' },
  ...
  { difficulty: 'hard', text: "Rust's ownership model prevents data races at compile time through three rules: each value has exactly one owner; ownership can be transferred (moved) or borrowed (&T / &mut T); mutable references are exclusive." },
  ...
  { difficulty: 'hard', text: "Category theory provides a unifying framework for mathematics through objects and morphisms. Functors map between categories preserving composition; natural transformations relate functors while respecting structure. Monads — endofunctors with unit & join — model side effects in pure languages." },
  { difficulty: 'hard', text: "The halting problem proves that no general algorithm can decide whether an arbitrary program terminates. Turing's 1936 proof uses diagonalization: assuming such a decider H exists leads to a program that halts iff H says it doesn't — a contradiction." },
